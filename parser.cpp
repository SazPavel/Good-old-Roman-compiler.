...
string masStoi(string str, string *count){//делит лексему массив на им€ массива и элемент
	int found = str.find("[");
	int found2 = str.find("]");
	string counts = str.substr(found+1, found2-2);
	*count = counts;
...
//	printf("exp\n");
	...
	node *n = new node;
	n = test();
	if((n->Type == TyInt || n->Type == TyFloat || n->Type == TyStringname || n->Type == TyString || n->Type == TyMasI || n->Type == TyMasF) && token->type == TyEqual){//если присвоение значени€
		node *n1 = new node;
		token_old = token->lexeme;
		*token = lexer->GetToken();
		cop(n1, n);
